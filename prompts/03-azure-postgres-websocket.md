...
- Create two tables on startup:
  ...
  - `analyses` — id, user_id, region, resources_scanned (int), issues_found (int), estimated_savings (text), analysis_result (jsonb), status, created_at
...
- During the `POST /api/analyze` flow, push progress messages through the WebSocket at each stage:
  ...
  - `"Scanning active services in <region>..."`
