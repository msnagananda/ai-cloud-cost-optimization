...
3. **Analysis Report** — Displays the AI analysis: summary, issues with severity badges, estimated monthly savings, and actionable AWS CLI fix commands in copyable code blocks.
4. **History** — Lists past analyses from `GET /api/history` with AWS region name, date, issues found, and estimated savings. Clicking one opens the full report.
